...
class labels:
    ...
    def get_personal_label(self, id):
        id_to_send = f"/{id}"
        res = self.my_api.api_get_request_by_id(self.data["url_label_api"], id_to_send)
        return res

    def create_new_personal_label(self, body):
        res = self.my_api.make_post_request(self.data["url_label_api"], body)
        return res

    def update_a_personal_label(self, id, body):
        id_to_send = f"/{id}"
        res = self.my_api.make_post_request_with_id(self.data["url_label_api"], id_to_send, body)
        return res

    def delete_label(self, id):
        id_to_send = f"/{id}"
        res = self.my_api.make_delete_request(self.data["url_label_api"], id_to_send)
        return res
